...
-- Columns
IFNULL (p25.total,0) "2025",
IFNULL (p24.total,0) "2024",
...
IFNULL (pTotal.total,0) "Total ('08-'25)"
-- End Columns
...
LEFT JOIN (SELECT "payee", SUM("amount") total FROM "alldata" WHERE ( "category" = 'Donations' AND "year" = 2025 AND status not in ('V')) GROUP BY "payee" ORDER BY "payee" COLLATE NOCASE ASC) p25 on p25.payee=a.payee
--2024
LEFT JOIN (SELECT "payee", SUM("amount") total FROM "alldata" WHERE ( "category" = 'Donations' AND "year" = 2024 AND status not in ('V')) GROUP BY "payee" ORDER BY "payee" COLLATE NOCASE ASC) p24 on p24.payee=a.payee
--2023
...
WHERE ( "category" = 'Donations'  AND "year" in (2008, 2009, 2010, 2011, 2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2020, 2021,2022,2023,2024,2025) AND status not in ('V')) GROUP BY "payee" ORDER BY "payee" COLLATE NOCASE ASC) pTotal on pTotal.payee=a.payee
UNION ALL
   SELECT  '<b>Annual Total:</b>',
  (SELECT TOTAL(amount) total FROM alldata WHERE year=2025 AND transactionType<>'Transfer' AND status <>'V' AND "category" = 'Donations') total17 , 
  (SELECT TOTAL(amount) total FROM alldata WHERE year=2024 AND transactionType<>'Transfer' AND status <>'V' AND "category" = 'Donations') total16 , 
